        ...
        implements ColumnReader
{
    ...
    {
        Block[] fieldBlocks = new Block[childColumnReaders.length];
        for (int i = 0; i < childColumnReaders.length; i++) {
            fieldBlocks[i] = childColumnReaders[i].readBlock();
        }
        return RowBlock.fromFieldBlocks(nextBatchSize, fieldBlocks);
    }

    @Override
    public DecodedPage read()
    {
        List<DecodedPage> decodedPages = Arrays.stream(childColumnReaders)
                .map(ColumnReader::read)
                .collect(toImmutableList());

        Block[] fieldBlocks = decodedPages.stream()
                .map(DecodedPage::getBlock)
                .toArray(Block[]::new);
        // The repetition/definition levels should be identical across all children

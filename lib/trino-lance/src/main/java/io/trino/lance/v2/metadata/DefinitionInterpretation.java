...
public enum DefinitionInterpretation
{
    ALL_VALID_ITEM, ALL_VALID_LIST, NULLABLE_ITEM, NULLABLE_LIST, EMPTYABLE_LIST, NULLABLE_AND_EMPTYABLE_LIST;

    public static DefinitionInterpretation fromProto(Encodings.RepDefLayer proto)
    {
        return switch (proto) {
    ...
    public static List<DefinitionInterpretation> fromProtoList(List<Encodings.RepDefLayer> protos)
    {
        return protos.stream().map(DefinitionInterpretation::fromProto).collect(toImmutableList());
    }

...
import io.trino.lance.v2.metadata.RepDefLayer;
...
import static io.trino.lance.v2.metadata.RepDefLayer.ALL_VALID_ITEM;
import static io.trino.lance.v2.metadata.RepDefLayer.NULLABLE_ITEM;
        ...
        implements RepetitionDefinitionUnraveler
{
    ...
    private final RepDefLayer[] interpretations;
    ...
    public BaseUnraveler(int[] repetitionLevels, int[] definitionLevels, RepDefLayer[] interpretations)
    {
        ...
        for (RepDefLayer interpretation : interpretations) {
    ...
    {
        return interpretations[currentLayer].isAllValid();
    }

    @Override
    public void skipValidity()
    {
        verify(interpretations[currentLayer] == ALL_VALID_ITEM);
        currentLayer++;
    }

    public long maxLists()
    {
        checkArgument(interpretations[currentLayer] != NULLABLE_ITEM, "Current layer must not be a struct or primitive");
    ...
    public BlockPositions calculateOffsets()
    {
        ...
        RepDefLayer currentMeaning = interpretations[currentLayer];
        switch (currentMeaning) {
            ...
            case ALL_VALID_LIST:
                ...
                break;
            default:
                throw new IllegalStateException("Unexpected definition interpretation for list layer");
        }
        currentLayer++;
        ...
        for (int i = currentLayer; i < interpretations.length; i++) {
            RepDefLayer level = interpretations[i];
            switch (level) {
                case NULLABLE_ITEM:
                    maxLevel++;
                    break;
    ...
    {
        if (interpretations[currentLayer] == ALL_VALID_ITEM) {
            currentLayer++;

        ...
        implements PageReader
{
    ...
    private final List<RepDefLayer> defInterpretations;
    ...
    public MiniBlockPageReader(LanceDataSource dataSource, Type type, MiniBlockLayout layout, List<DiskRange> bufferOffsets, long numRows)
    {
        ...
        this.maxVisibleDefinition = defInterpretations.stream().takeWhile(interpretation -> !interpretation.isList()).mapToInt(RepDefLayer::numDefLevels).sum();
    ...
    public void readChunk(ChunkMetadata chunk, RepIndexBlock chunkIndex, long rowsToSkip, long rowsToTake, PreambleAction preambleAction, boolean takeTrailer)
    {
        try {
            ...
            SelectedRanges selectedRanges = mapRange(Range.of(rowsToSkip, rowsToSkip + rowsToTake + (takeTrailer ? 1 : 0)),
                    ...
                    toIntExact(defInterpretations.stream().filter(RepDefLayer::isList).count()),

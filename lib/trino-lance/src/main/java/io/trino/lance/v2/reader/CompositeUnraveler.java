        ...
        implements RepetitionDefinitionUnraveler
{
    ...
    {
        this.unravelers = requireNonNull(unravelers);
    }
    ...
    public Optional<boolean[]> calculateNulls()
    {
        if (isAllValid()) {
            ...
            BooleanArrayList validity = unravelers.stream()
                    // FIXME(hluo): this needs some more testing or clarification from lance if it's possible to have some pages being all valid but some not

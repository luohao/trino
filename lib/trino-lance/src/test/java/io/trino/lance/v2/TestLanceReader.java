...
public class TestLanceReader
{
            ...
            throws Exception
    {
        ...
        testRoundTripNumeric(limit(cycle(ImmutableList.of(1, 3, 5, 7, 11, 13, 17, 19, 23, 27)), 1024));
    }
    ...
    public void testLargeNumeric()
            throws Exception
    {
        // it seems that Lance writer always do bitpacking even for large numbers
            ...
            throws Exception
    {
        ...
        tester.testRoundTrip(TINYINT, writeValues.stream().map(Long::byteValue) // truncate values to byte range
                .collect(toList()));

        tester.testRoundTrip(SMALLINT, writeValues.stream().map(Long::shortValue) // truncate values to short range
                .collect(toList()));

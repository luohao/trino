...
import io.trino.plugin.lancedb.catalog.BaseTable;
...
public class LanceDbMetadata
        implements ConnectorMetadata
{
    private final TrinoCatalog catalog;

    @Inject
    public LanceDbMetadata(TrinoCatalog catalog)
    {
            ...
            Optional<ConnectorTableVersion> endVersion)
    {
        ...
        return new LanceDbTableHandle(tableName, baseTable.loadManifest(version), baseTable.getTableLocation().toString());
    }

    @Override
    public ConnectorTableMetadata getTableMetadata(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        checkArgument(tableHandle instanceof LanceDbTableHandle);
        LanceDbTableHandle table = (LanceDbTableHandle) tableHandle;
    ...
    public Map<String, ColumnHandle> getColumnHandles(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        ...
        return table.manifest().getFields().stream()
                .collect(toImmutableMap(Field::getName,
                        field -> new LanceDbColumnHandle(field.getId(), field.getName(), field.toTrinoType())));
    }
    ...
    {
        checkArgument(tableHandle instanceof LanceDbTableHandle);
        LanceDbColumnHandle column = (LanceDbColumnHandle) columnHandle;

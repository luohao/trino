 ...
 */
package io.trino.plugin.lancedb.catalog;
...
import io.trino.plugin.lancedb.metadata.Manifest;
...
import static io.trino.plugin.lancedb.LanceDbErrorCode.LANCEDB_INVALID_VERSION_NUMBER;
...
public class BaseTable
{
    ...
    public Manifest loadManifest(Optional<Long> version)
    {
        ...
        if (manifestLocation.isEmpty()) {
            throw new TrinoException(LANCEDB_INVALID_VERSION_NUMBER, format("Manifest not found for version: %s", version));
        }
